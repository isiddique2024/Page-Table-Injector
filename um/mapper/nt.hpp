...
#define MiGetVirtualAddressMappedByPte(PTE)                                                      \
  ((std::uintptr_t)(                                                                             \
      (LONG_PTR)(((LONG_PTR)(PTE)-0xFFFFF68000000000) << (PAGE_SHIFT + VA_SHIFT - PTE_SHIFT)) >> \
      VA_SHIFT))
...
namespace nt {
  ...
  typedef NTSTATUS (*RtlAdjustPrivilege)(_In_ ULONG Privilege, _In_ BOOLEAN Enable,
  ...
  typedef struct _DRIVER_OBJECT {
    ...
    LONG(*DriverInit)
    (struct _DRIVER_OBJECT* arg1, struct _UNICODE_STRING* arg2);            // 0x58
    ...
    (struct _DEVICE_OBJECT* arg1, struct _IRP* arg2);  // 0x70
  } DRIVER_OBJECT;
  ...
  // Thanks to Pvt Comfy for remember to update this
...
extern "C" NTSTATUS NtFreeVirtualMemory(HANDLE ProcessHandle, PVOID* BaseAddress,
