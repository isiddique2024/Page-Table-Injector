  ...
  #define Log(content)                                                      \
    do {                                                                    \
      ...
      if (_wcontent.length() > 3) {                                         \
        std::wcout << _wcontent.substr(0, 3) << L" " << __FUNCTION__ << L"" \
                   << _wcontent.substr(3);                                  \
      } else {                                                              \
...
namespace utils {
  ...
  bool CreateFileFromMemory(const std::wstring& desired_file_path, const char* address,
                            size_t size);
  ...
  auto init_mapper_dependencies() -> NTSTATUS;
}  // namespace utils

std::wstring utils::GetFullTempPath() {
  wchar_t temp_directory[MAX_PATH + 1] = {0};
  const uint32_t get_temp_path_ret =
      shadowcall<DWORD>("GetTempPathW", sizeof(temp_directory) / 2, temp_directory);
...
bool utils::ReadFileToMemory(const std::wstring& file_path, std::vector<uint8_t>* out_buffer) {
  ...
  out_buffer->assign((std::istreambuf_iterator<char>(file_ifstream)),
...
bool utils::CreateFileFromMemory(const std::wstring& desired_file_path, const char* address,
                                 size_t size) {
...
__forceinline auto utils::init_mapper_dependencies() -> NTSTATUS {
  ...
  status = shadowsyscall<NTSTATUS>("NtOpenProcessToken", ((HANDLE)(LONG_PTR)-1),
                                   TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken);

  if (!NT_SUCCESS(status)) {
    Log(L"[!] Failed to open process token" << std::endl);
    return nt::STATUS_UNSUCCESSFUL;
  }

  TOKEN_PRIVILEGES tkp_debug;
  tkp_debug.PrivilegeCount = 1;
  tkp_debug.Privileges[0].Luid.LowPart = 0x14;  // SeDebugPrivilege
  tkp_debug.Privileges[0].Luid.HighPart = 0;
  tkp_debug.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;

  status = shadowsyscall<NTSTATUS>("NtAdjustPrivilegesToken", hToken, FALSE, &tkp_debug,
                                   sizeof(TOKEN_PRIVILEGES), NULL, NULL);

  if (!NT_SUCCESS(status)) {
    Log(L"[!] Failed to set SeDebugPrivilege" << std::endl);
    NtClose(hToken);
    return nt::STATUS_UNSUCCESSFUL;
  }

  TOKEN_PRIVILEGES tkp_lmp;
  tkp_lmp.PrivilegeCount = 1;
  tkp_lmp.Privileges[0].Luid.LowPart = 0x4;  // SeLockMemoryPrivilege
  tkp_lmp.Privileges[0].Luid.HighPart = 0;
  tkp_lmp.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;

  status = shadowsyscall<NTSTATUS>("NtAdjustPrivilegesToken", hToken, FALSE, &tkp_lmp,
                                   sizeof(TOKEN_PRIVILEGES), NULL, NULL);

  if (!NT_SUCCESS(status)) {
    Log(L"[!] Failed to set SeLockMemoryPrivilege" << std::endl);
    NtClose(hToken);
    return nt::STATUS_UNSUCCESSFUL;
  }
...
uint64_t utils::GetKernelModuleAddress(const std::string& module_name) {
  ...
  for (auto i = 0u; i < modules->NumberOfModules; ++i) {
    const std::string current_module_name =
        std::string(reinterpret_cast<char*>(modules->Modules[i].FullPathName) +
...
uintptr_t utils::FindPattern(uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask,
                             const char* szMask) {
...
PVOID utils::FindSection(const char* sectionName, uintptr_t modulePtr, PULONG size) {
  ...
  PIMAGE_NT_HEADERS headers =
      (PIMAGE_NT_HEADERS)(modulePtr + ((PIMAGE_DOS_HEADER)modulePtr)->e_lfanew);
