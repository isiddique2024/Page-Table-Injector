...
public:
    ...
    inline [[nodiscard]] auto initialize() -> bool {
        ...
        NtConvertBetweenAuxiliaryCounterAndPerformanceCounter = reinterpret_cast<NtConvertBetweenAuxiliaryCounterAndPerformanceCounter_t>(
            shadowcall<FARPROC>("GetProcAddress", ntdll, "NtConvertBetweenAuxiliaryCounterAndPerformanceCounter")
            );
        return NtConvertBetweenAuxiliaryCounterAndPerformanceCounter != nullptr;
    }

    inline [[nodiscard]] auto unload() -> bool {
        unload_request data{};
        bool buffer = false;
        data.buffer = &buffer;
    ...
    inline [[nodiscard]] auto get_module_base(std::uint32_t pid, const std::wstring& module_name) -> std::uintptr_t {
        base_request data{};
        data.pid = pid;
        data.handle = 0;
    ...
    inline auto find_signature(std::uint32_t pid, const std::wstring& mod_name, const std::string& signature) -> std::uintptr_t {
    ...
    inline auto read_virtual_memory(std::uint32_t pid, std::uintptr_t address, void* buffer, std::size_t size) -> bool {
    ...
    inline auto write_virtual_memory(std::uint32_t pid, std::uintptr_t address, const void* buffer, std::size_t size) -> bool {
        write_request data{ pid, address, const_cast<void*>(buffer), size };
        return send_driver_request(&data, request_codes::write);
    }

    template <typename T>
    inline auto write_virtual_memory(std::uint32_t pid, std::uintptr_t address, const T& value) -> bool {
        return write_virtual_memory(pid, address, &value, sizeof(T));
    }

    inline auto allocate_independent_pages(std::uint32_t local_pid, std::uint32_t target_pid, std::uint32_t target_tid, std::size_t size, bool use_large_pages, std::uint32_t alloc_mode) -> void* {
        allocate_independent_pages_request data{ local_pid, target_pid, target_tid, nullptr, size, use_large_pages, alloc_mode };
        send_driver_request(&data, request_codes::allocate_independent_pages);
        return data.address;
    }

    inline auto execute_dll_via_thread(std::uint32_t local_pid, std::uint32_t target_pid, std::uint32_t target_tid, void* address, unsigned int entry_point, std::uint32_t alloc_mode) -> bool {
        execute_dll_via_thread_request data{ local_pid, target_pid, target_tid, address, entry_point, alloc_mode };
...
private:
    using NtConvertBetweenAuxiliaryCounterAndPerformanceCounter_t = NTSTATUS(__fastcall*)(uintptr_t, void*, void*, void*);
    NtConvertBetweenAuxiliaryCounterAndPerformanceCounter_t NtConvertBetweenAuxiliaryCounterAndPerformanceCounter = nullptr;
    ...
    inline bool send_driver_request(void* data, request_codes code) {
        ...
        auto result = shadowsyscall<NTSTATUS>("NtConvertBetweenAuxiliaryCounterAndPerformanceCounter",
            1,
            &auxiliary_counter,
            &perf_counter,
            nullptr
