...
int main(int argc, char* argv[]) {
  ...
  auto driver_memory_type_map = std::map<std::string, std::uint32_t>{
      {"normal", nt::NORMAL_PAGE}, {"large", nt::LARGE_PAGE}, {"huge", nt::HUGE_PAGE}};
  ...
  auto dll_memory_map =
      std::map<std::string, std::uint32_t>{{"normal", driver->memory_type::NORMAL_PAGE},
...
Note: Use quotes around window names with spaces
  ...
  if (driver_mem_type == nt::HUGE_PAGE || dll_mem_type == driver->memory_type::HUGE_PAGE) {
    ...
    if (driver_mem_type == nt::HUGE_PAGE)
      driver_mem_type = nt::LARGE_PAGE;
    if (dll_mem_type == driver->memory_type::HUGE_PAGE)
      dll_mem_type = driver->memory_type::LARGE_PAGE;
  }

  // warn about dangerous hide options
  if (driver_hide_type == nt::SET_LOCK_BIT || dll_hide_type == nt::SET_LOCK_BIT) {
    log("WARNING", "set_lock_bit is an anti-debug mechanism that will crash the system if "
  ...
  if (!kdmapper::Init(static_cast<nt::driver_alloc_mode>(driver_alloc_mode),
                      static_cast<nt::memory_type>(driver_mem_type),
                      static_cast<nt::hide_type>(driver_hide_type),
                      static_cast<nt::hide_type>(dll_hide_type))) {
  ...
  if (execution_method == "iat") {
    injector->set_iat_hook_params(hook_module.c_str(), hook_function.c_str(),
  ...
  if (!injector->run(pid, tid, dll_bytes, dll_size,
                     static_cast<driver_t::memory_type>(dll_mem_type),
                     static_cast<driver_t::alloc_mode>(dll_alloc_mode))) {
