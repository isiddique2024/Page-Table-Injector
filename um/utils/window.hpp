...
public:
  ...
  std::optional<HWND> find_window(const std::wstring& target_class_name) {
    ...
    nt_enum_windows(
        [](HWND hwnd, LPARAM lParam) -> BOOL {
          ...
          shadowcall<int>({"GetClassNameW", "user32.dll"}, hwnd, class_name.data(),
                          static_cast<int>(class_name.size()));
          std::array<wchar_t, 256> window_title{};
          shadowcall<int>({"GetWindowTextW", "user32.dll"}, hwnd, window_title.data(),
  ...
  bool initialize_and_find_window(std::wstring window_name, std::uint32_t& process_id,
...
private:
  bool nt_enum_windows(WNDENUMPROC lp_enum_func, LPARAM l_param) {
    ...
    auto nt_status = shadowcall<NTSTATUS>({"NtUserBuildHwndList", "win32u.dll"}, nullptr, nullptr,
                                          false, false, 0, static_cast<unsigned int>(hwnds.size()),
                                          hwnds.data(), &hwnds_found);
    if (nt_status == 0xC0000023) {
      hwnds.resize(hwnds_found);
      nt_status = shadowcall<NTSTATUS>({"NtUserBuildHwndList", "win32u.dll"}, nullptr, nullptr,
                                       false, false, 0, static_cast<unsigned int>(hwnds.size()),
                                       hwnds.data(), &hwnds_found);
    }
