...
namespace hyperspace {
  ...
  auto copy_ntoskrnl_pages(uintptr_t src_base, uintptr_t size, uintptr_t dest_pd_pa) -> bool {
    ...
    uintptr_t* page_pas =
        reinterpret_cast<uintptr_t*>(mem::allocate_independent_pages(page_pas_size));
  ...
  auto initialize_cloned_eprocess_lists(PEPROCESS clone_eproc) -> void {
    ...
    InitializeListHead(shared_commit_head);

    // clear _EPROCESS.SharedCommitCharge
    InterlockedExchange64(reinterpret_cast<LONG64*>(reinterpret_cast<uintptr_t>(clone_eproc) +
                                                    globals::_eprocess_shared_commit_charge),
                          0);
    ...
    // clear AccessLog to prevent MiEmptyPageAccessLog
  ...
  auto initialize_hyperspace_context(uint32_t target_pid, hyperspace_ctx* ctx) -> NTSTATUS {
    ...
    // copy original PML4 with proper self-reference entry handling, i'm not sure
    // if this is truly necessary, but I did it anyway
    self_reference_entry_info self_reference_info;
  ...
  auto allocate_in_hyperspace(uint32_t target_pid, size_t size, bool use_large_page) -> void* {
    ...
    // set the search range, for simplicity sake i'm just going to default to low
    // address allocation within the hyperspace ctx
    ...
    for (uint32_t idx = start_idx; idx < end_idx; idx++) {
      physical::read_physical_address(target_dir_base + idx * sizeof(PML4E_64), &pml4e,
                                      sizeof(PML4E_64));
      if (!pml4e.Present) {
        available_count++;
      }
  ...
  auto switch_thread_context_to_hyperspace(uint32_t tid, hyperspace_ctx* ctx) -> NTSTATUS {
    ...
    // swap the thread's _KTHREAD.ApcState.Process to the cloned PEPROCESS
  ...
  auto switch_from_hyperspace(uint32_t tid, hyperspace_ctx* ctx) -> bool {
    ...
    // restore the thread's _KTHREAD.ApcState.Process to the original PEPROCESS
    PEPROCESS* apc_process_ptr = reinterpret_cast<PEPROCESS*>(
        reinterpret_cast<uintptr_t>(target_thread) + globals::_kthread_apcstate_pkprocess);
  ...
  auto cleanup_hyperspace_context(hyperspace_ctx* ctx) -> void {
    ...
    // if (ctx->clone_peproc_page_base) {
    //    globals::mm_free_independent_pages(ctx->clone_peproc_page_base,
    //    PAGE_SIZE);
    //}
  ...
  namespace callbacks {
    ...
    auto process_notify_callback_impl(HANDLE ParentId, HANDLE ProcessId, BOOLEAN Create) -> void {
      ...
      // clean up contextualized ntoskrnl copy in hyperspace ctx (need to rewrite
      // since this bsods, most likely due to calling MmFreeIndependentPages on 2MB
      // contiguous memory, will have to switch to 4kb independent pages for
