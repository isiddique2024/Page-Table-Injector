...
namespace {
  ...
  public:
    ...
    static auto handle(void* a1, std::int64_t* status) -> std::int64_t {
      ...
      char previous_mode = *reinterpret_cast<char*>(
          reinterpret_cast<std::uintptr_t>(current_thread) + 0x232);  // PreviousMode
  ...
  private:
    ...
    static auto handle_write_request(request_data* request) -> std::int64_t {
      ...
      if (globals::ctx.initialized) {
        status = physical::copy_memory(globals::io_get_current_process(),
                                       ...
                                       globals::ctx.clone_peproc, (void*)data.address, data.size);

      } else {
        status = physical::copy_memory(globals::io_get_current_process(),
                                       reinterpret_cast<void*>(data.buffer), target_process,
                                       (void*)data.address, data.size);
      }
    ...
    static auto handle_read_request(request_data* request) -> std::int64_t {
      ...
      if (globals::ctx.initialized) {
        status = physical::copy_memory(globals::ctx.clone_peproc, (void*)data.address,
                                       globals::io_get_current_process(),
                                       reinterpret_cast<void*>(data.buffer), data.size);
      } else {
        status = physical::copy_memory(target_process, (void*)data.address,
                                       globals::io_get_current_process(),
                                       reinterpret_cast<void*>(data.buffer), data.size);
      }
    ...
    static auto handle_execute_dll_via_thread_request(request_data* request) -> std::int64_t {
      ...
      *(std::uintptr_t*)((std::uintptr_t)kernel_shellcode + dll_base_offset) =
          (std::uintptr_t)data.alloc_base;

      // patch the entry point address
      *(std::uintptr_t*)((std::uintptr_t)kernel_shellcode + entry_point_offset) =
      ...
      if (!remote_shellcode) {
        ...
        globals::mm_free_independent_pages(reinterpret_cast<uintptr_t>(kernel_shellcode),
                                           PAGE_SIZE);
      ...
      if (globals::ctx.initialized) {
        write_status = physical::copy_memory(globals::io_get_current_process(), kernel_shellcode,
                                             globals::ctx.clone_peproc, remote_shellcode,
                                             sizeof(dll_main_shellcode));
      } else {
        write_status =
            physical::copy_memory(globals::io_get_current_process(), kernel_shellcode,
      ...
      NTSTATUS open_status = globals::zw_open_process(&process_handle, PROCESS_ALL_ACCESS,
                                                      &obj_attr, &process_client_id);
      ...
      NTSTATUS thread_status =
          globals::rtl_create_user_thread(process_handle,    // Target process handle
                                          ...
                                          &thread_handle, &thread_client_id);
      ...
      if (globals::ctx.initialized) {
        ...
        if (!NT_SUCCESS(swap_context)) {
          log("ERROR",
              "hyperspace::switch_thread_context_to_hyperspace failed with "
              "status: 0x%X",
              swap_context);
      ...
      if (thread_handle) {
        ...
        NTSTATUS wait_status = globals::zw_wait_for_single_object(thread_handle, FALSE, &timeout);
      ...
      if (globals::ctx.initialized) {
        ...
        if (globals::ps_lookup_process_by_process_id(reinterpret_cast<HANDLE>(data.target_pid),
          ...
          physical::copy_memory(globals::io_get_current_process(), kernel_shellcode,
                                cleanup_process, remote_shellcode, sizeof(dll_main_shellcode));
      ...
      // free remote shellcode bsod when freeing this for some reason lol
      // globals::mm_free_independent_pages(reinterpret_cast<uintptr_t>(remote_shellcode),
      // PAGE_SIZE);
...
auto entry(uintptr_t io_get_current_process_addr, uintptr_t mm_copy_virtual_memory_addr,
  ...
  auto status_hide =
      init::hide_driver_pages(local_offsets.DriverAllocBase, local_offsets.DriverSize);
